...
class Control:
    def __init__(self):
        ...
        self.imu = IMU()
        self.claws = Claws()
        self.servo = self.claws.servo # Servo()
        ...
        #claws from 60 to 110  or 120 to 70  arms from 40 to 90 or from 90 to 140
        #wide and open  left is 140 and 110
        #               right is 40  and 70
        self.claws.leftClawArm(140)
        self.claws.leftClaw(110)
        self.claws.rightClawArm(40)
        self.claws.rightClaw(70)
        #increasing or decreasing angle
        self.arm_angle_count = 0  
        self.arm_inc = False
        self.claw_angle_count = 0  
        self.claw_inc = False
        ...
        self.scanAngle = 90
        self.scanUp = True
        self.scanForwardDist = 10
        self.scanMidLeftDist = 10
        self.scanFarLeftDist = 10
        self.scanMidRightDist = 10
        self.scanFarRightDist = 10

        self.Thread_conditiona = threading.Thread(target=self.condition)


    def logScanDistance(self):
        # working on an average of 5

        if self.scanAngle == 10: self.scanFarRightDist = int((self.ultrasonic.getDistance() + self.ultrasonic.getDistance() + self.ultrasonic.getDistance() + self.ultrasonic.getDistance() + self.ultrasonic.getDistance() )/5)
        elif self.scanAngle == 50: self.scanMidRightDist = int((self.ultrasonic.getDistance() + self.ultrasonic.getDistance() + self.ultrasonic.getDistance() + self.ultrasonic.getDistance() + self.ultrasonic.getDistance() )/5)
        elif self.scanAngle == 90: self.scanForwardDist = int((self.ultrasonic.getDistance() + self.ultrasonic.getDistance() + self.ultrasonic.getDistance() + self.ultrasonic.getDistance() + self.ultrasonic.getDistance() )/5)
        elif self.scanAngle == 130: self.scanMidLeftDist = int((self.ultrasonic.getDistance() + self.ultrasonic.getDistance() + self.ultrasonic.getDistance() + self.ultrasonic.getDistance() + self.ultrasonic.getDistance() )/5)
        elif self.scanAngle == 170: self.scanFarLeftDist = int((self.ultrasonic.getDistance() + self.ultrasonic.getDistance() + self.ultrasonic.getDistance() + self.ultrasonic.getDistance() + self.ultrasonic.getDistance() )/5)
    ...
    def coordinateTransformation(self, point):
        ...
        # leg4
        self.leg_point[3][0] = point[3][0] * math.cos(-126 / 180 * math.pi) + point[3][1] * math.sin(
            -126 / 180 * math.pi) - 94
    ...
    def run(self, data, Z=40, F=64):  # example : data=['CMD_MOVE', '1', '0', '25', '10', '0']
        ...
        for i in range(6):
            xy[i][0] = ((point[i][0] * math.cos(angle / 180 * math.pi) + point[i][1] * math.sin(angle / 180 * math.pi) -
                         point[i][0]) + x) / F
    ...
    def claws_move(self):
        if self.arm_angle_count >= 50:
            self.arm_inc = False
        elif self.arm_angle_count <= 0:
            self.arm_inc = True
        if self.arm_inc:
            self.arm_angle_count += 2
        else:
            self.arm_angle_count -= 2
        if self.claw_angle_count >= 50:
            self.claw_inc = False
        elif self.claw_angle_count <= 0:
            self.claw_inc = True
        if self.claw_inc:
            self.claw_angle_count += 4
        else:
            self.claw_angle_count -= 4
            
        self.claws.leftClawArm(140 - self.claw_angle_count)
        self.claws.leftClaw(110 - self.claw_angle_count)
        self.claws.rightClawArm(40 + self.arm_angle_count)
        self.claws.rightClaw(70 + self.arm_angle_count)
            
    def head_sweep(self):
        if self.scanAngle >= 170:
            self.scanUp = False
        elif self.scanAngle <= 10:
            self.scanUp = True
        if self.scanUp:
            self.scanAngle += 5
        else:
            self.scanAngle -= 5
        self.servo.setServoAngle(1, self.scanAngle)
        # critical angles are 10 50 90 130 and 180
        if self.scanAngle == 10 or self.scanAngle == 50 or self.scanAngle == 90 or self.scanAngle == 130 or self.scanAngle == 170:
            self.logScanDistance()
